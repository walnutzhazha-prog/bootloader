...
void Int_w25q128_erase_sector(uint8_t block, uint8_t sector)
{
    ...
    //2.拉低片选引脚，启动SPI设备
    Int_w25q128_start();
    uint32_t addr_24 = (uint32_t)block * 65536 + (uint32_t)sector * 4096;
    //3.发送扇区擦除的命令和地址
    Int_w25q128_write_byte(W25Q128_ERASE_SECTOR);
    //4.发送地址
    Int_w25q128_write_byte(block << 16);    // 地址的高8位
    Int_w25q128_write_byte(sector << 12);   // 地址的中8位
    Int_w25q128_write_byte(0);              // 地址的低8
    //5.拉高片选引脚，停止SPI设备
    Int_w25q128_stop();
}
